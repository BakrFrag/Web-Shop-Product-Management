...
from apis.schemas import ProductCreate, ProductResponse, ProductUpdate
...
async def create_new_product(
    ...
    """
    return await create_product(db, product.name, product.description, product.price, product.stock_quantity)
...
async def list_all_products(
    ...
    """
    return await get_products_list(db)
...
async def update_product_by_id(
    ...
    Args:
        ...
        update_product_data (ProductUpdate): data to update product 
        db (Session): db session object
        current_user: current login user
    """
    return await update_product(db, product_id, update_product_data.model_dump(exclude_none = True))